var $ = {
  ...
  state: {
    ...
    distance: 0,   // ✅ new
    score: 0,      // ✅ new
    time: 0,       // ✅ new (for day/night cycle)
    dayLength: 2000
  },
  storage: {
    bg: null
...
$.state.time = ($.state.time + 1) % $.state.dayLength;
    ...
    drawScore($.ctx, 30, 40, $.state.score); // ✅ add this
...
function calcMovement() {
  ...
  // increase distance based on speed
...
function drawSky() {
  let dayProgress = $.state.time / $.state.dayLength;
  let brightness = Math.sin(dayProgress * Math.PI * 2) * 0.5 + 0.5; 

  // interpolate between day and night colors
  let dayColor = {r: 212, g: 245, b: 254};  // light blue
  let nightColor = {r: 10, g: 20, b: 50};   // dark blue

  let r = Math.floor(dayColor.r * brightness + nightColor.r * (1 - brightness));
  let g = Math.floor(dayColor.g * brightness + nightColor.g * (1 - brightness));
  let b = Math.floor(dayColor.b * brightness + nightColor.b * (1 - brightness));
